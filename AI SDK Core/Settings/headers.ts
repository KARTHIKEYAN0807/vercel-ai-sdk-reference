...
async function main() {
  ...
  try {
    const result = await generateText({
      ...
      // The Headers: These are injected directly into the underlying HTTP request.
      // Very useful for tracking proxies, organization IDs, or custom metadata.
      headers: {
    ...
    // 3. Flushes exactly what is needed to Langfuse, then safely closes Node
